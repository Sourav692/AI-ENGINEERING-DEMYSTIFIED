# Financial Compliance Document Reviewer - Answer Key
...
- What exact financial services compliance review workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: analyst uploads marketing/client/regulatory document, assistant detects policy gaps, cites rules, classifies risk, drafts remediation notes, and routes high-risk cases for human approval.
...
- Connectors ingest and normalize data from document management system, policy repository, regulatory rule library, GRC platform, case management, identity provider; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
- Document(id, type, jurisdiction, business_unit, version, content_hash); Rule(id, jurisdiction, product, obligation_text, effective_date, severity); Finding(id, document_id, rule_id, evidence_span, severity, status); ReviewerDecision(id, finding_id, decision, comment); AuditLog(actor, doc_id, rule_ids, model_version, timestamp).
...
## Red-team risks
- missed compliance violation, outdated regulation, hallucinated rule, unauthorized document access, over-automation of legal judgment
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real financial services compliance review workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
