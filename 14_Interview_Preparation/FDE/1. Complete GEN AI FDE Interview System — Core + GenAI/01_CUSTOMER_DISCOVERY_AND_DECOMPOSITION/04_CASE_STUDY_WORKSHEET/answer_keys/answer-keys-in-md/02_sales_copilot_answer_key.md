# Sales Copilot for Account Executives - Answer Key
...
- What exact B2B sales productivity workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: AE prepares for account meeting, asks for account summary, opportunity risks, suggested discovery questions, email drafts, and CRM update suggestions.
...
- Connectors ingest and normalize data from Salesforce/HubSpot, Gong/Zoom transcripts, emails/calendar, product docs, pricing/discount policy, customer support history, data warehouse; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
## Red-team risks
- invented customer facts, disallowed pricing promises, confidential account leakage, biased lead scoring, prompt injection from email/transcript content
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real B2B sales productivity workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
