# Full Premium Mock Interview: Financial Compliance + Red Team
...
## Customer scenario
A regulated financial-services firm wants an assistant that reviews advisor communications, detects possible compliance violations, and helps analysts prepare review notes. The demo caught obvious banned phrases, but missed subtle promissory language and was vulnerable to instructions embedded inside emails.

## Ambiguous customer request
> “We want the AI to review all advisor messages and tell us which ones are risky.”

## Strong candidate target framing
- Primary workflow: compliance analysts reviewing emails, chat messages, marketing drafts, and advisor notes for suitability, promissory language, disclosure gaps, and escalation needs
- Users: compliance analysts, surveillance managers, legal reviewers, model risk, audit, advisor supervisors
- Success metrics: recall on high-risk violations, false-positive review burden, citation/evidence quality, escalation precision, prompt-injection resilience, audit completeness, model drift by product line
- Architecture direction: immutable message ingestion, policy taxonomy, rules + ML prefilters, RAG over approved compliance manuals, LLM evidence extractor, risk scorer, red-team harness, analyst review queue, audit log, model/version registry
- Security boundary: no autonomous regulatory decisions, immutable evidence trace, adversarial content isolation, strict prompt boundaries, retention controls, PII/financial data controls, approval workflow, audit export
- Latency thinking: batch-review overnight for bulk surveillance; interactive p95 under 10 seconds for analyst drill-down; precompute embeddings and policy matches; prioritize high-risk queues
- Cost thinking: tiered screening: rules and small classifiers for bulk, LLM only for borderline cases; sample low-risk traffic; track cost per reviewed communication and cost per true issue found
- Rollout direction: backtest on historical regulatory findings, shadow mode next to analysts, calibrate thresholds by risk category, audit sign-off, limited production by business unit
...
**Interviewer:** That sounds high level. The customer says: “We want the AI to review all advisor messages and tell us which ones are risky.” What do you do in the first meeting?
...
**Candidate:** For this case, I would focus the first release on compliance analysts reviewing emails, chat messages, marketing drafts, and advisor notes for suitability, promissory language, disclosure gaps, and escalation needs. The relevant users are compliance analysts, surveillance managers, legal reviewers, model risk, audit, advisor supervisors. The first dashboard should track recall on high-risk violations, false-positive review burden, citation/evidence quality, escalation precision, prompt-injection resilience, audit completeness, model drift by product line. The security controls should emphasize no autonomous regulatory decisions, immutable evidence trace, adversarial content isolation, strict prompt boundaries, retention controls, PII/financial data controls, approval workflow, audit export. I would not index every possible source at the beginning; I would start with authoritative sources and named owners.
...
**Candidate:** I would agree that this is a real risk and explain the controls. The LLM should only receive context that the user is authorized to see. Permissions must be enforced in retrieval and tool execution, not only in the prompt. Logs should avoid storing sensitive raw content unless required and protected. We need audit trails, data retention controls, prompt-injection tests, and clear blocked actions. For this mock, the core security boundary is: no autonomous regulatory decisions, immutable evidence trace, adversarial content isolation, strict prompt boundaries, retention controls, PII/financial data controls, approval workflow, audit export.
...
**Candidate:** I would avoid arguing in abstract. I would calculate cost per successful workflow outcome. For this use case, the cost strategy is: tiered screening: rules and small classifiers for bulk, LLM only for borderline cases; sample low-risk traffic; track cost per reviewed communication and cost per true issue found. I would show a cost dashboard with request volume, model distribution, average tokens, tool calls, cache hit rate, and cost per accepted answer or resolved case. If the assistant saves time but costs more than the value created, the architecture is wrong.
...
**Candidate:** I would push back respectfully. I would say we can launch a controlled pilot next Monday, but not broad production unless the launch gates are met. My rollout would be: backtest on historical regulatory findings, shadow mode next to analysts, calibrate thresholds by risk category, audit sign-off, limited production by business unit. I would also define rollback conditions, such as leakage, high-risk wrong answers, excessive latency, or low user trust.
...
**Weak candidate:** I would use an LLM to read each message and classify it as compliant or non-compliant. Then compliance can review the flagged ones.
...
**Average candidate:** I would combine rules, retrieval from compliance policy, and human review. I would evaluate precision and recall and add red-team tests for prompt injection.
...
**Candidate:** I would frame this as regulated decision support, not autonomous compliance judgment. The system should extract evidence, map it to policy clauses, propose risk labels with confidence, and preserve an audit trail. Launch requires high recall on severe violations, controlled false positives, and explicit resistance to adversarial instructions inside reviewed communications.
