...
- **Same rubric, same verdict as the full chapter** — this is a compression, not a re-analysis. Nothing was added or dropped from the coverage assessment.
- **Fully covered (17):** outcome reframing; stakeholder mapping; architecture-changing questions; must/should/could requirements; explicit non-goals; capacity math; unit economics; end-to-end architecture; data model and API contracts; balanced trade-off pairs; **build-vs-buy** (an explicit gap in the Chapter 2 tutorial — this scenario is its natural home); threat model; failure drills; testing strategy; metrics and observability; phased rollout with risk register; communication plan and rubric.
- **Partial (2):** *Regulatory depth* — retention, hold, discovery and works-council consultation are first-class here, but named framework mechanics (GDPR lawful basis for employee monitoring, FINRA supervision, DPA clause detail) are not worked through. *Responsible-AI beyond safety filtering* — bias in an employee-facing assistant (e.g. systematically different career guidance across groups) is only implicit in the eval-set discussion.
- **Absent (1):** *Multilingual and accessibility design* — a global workforce is not monolingual, and retrieval, policy classification and evaluation all degrade unevenly across languages; screen-reader behavior for a streaming response is a real unaddressed problem.
- Gaps are named honestly rather than given token paragraphs to force a checkmark.

---

*Condensed companion to `chapter-21-enterprise-chatbot-platform-tutorial.md`. Custom addition to the 20-scenario set, in the FDE Chapter Tutorial format — not derived from the source book's chapter list.*
