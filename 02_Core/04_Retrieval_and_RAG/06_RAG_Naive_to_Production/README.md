...
Some notebooks reference a shared `data/` folder via relative paths from the source repo — see `02_Core/04_Retrieval_and_RAG/shared_data/` (brought along, but exact relative-path depth wasn't re-verified after the move; fix paths if a notebook can't find its data file).
