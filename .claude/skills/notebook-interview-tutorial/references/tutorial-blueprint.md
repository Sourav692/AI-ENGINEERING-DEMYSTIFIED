...
Both files live in `06_Interview_Prep/Study_Guides/`, side by side, so git tracks the rendered page as well
...
Path: `06_Interview_Prep/Study_Guides/<slug>_INTERVIEW_TUTORIAL.md`, where `<slug>` is the input folder
...
Path: `06_Interview_Prep/Study_Guides/<slug>_INTERVIEW_TUTORIAL.html` — the same `<slug>`, beside the
