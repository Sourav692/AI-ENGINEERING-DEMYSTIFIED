# Healthcare Prior Authorization Assistant - Answer Key
...
- What exact healthcare prior authorization workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: staff prepares prior-auth package by checking payer policy, patient record facts, clinical criteria, missing documents, and drafts a submission for clinician approval.
...
- Connectors ingest and normalize data from EHR via FHIR, payer policy portals, document store, claims/prior-auth system, identity provider, audit/compliance logs; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
- Patient(id, mrn_hash, demographics_minimized, consent_flags); Encounter(id, diagnosis_codes, procedure_codes, clinician_notes); PayerPolicy(id, payer, plan, criteria, effective_date); AuthRequest(id, patient_id, procedure, status, required_evidence); EvidenceLink(request_id, source_record, field, confidence); AuditLog(actor, patient_id, access_reason, output_id).
...
## Red-team risks
- PHI leakage, incorrect medical necessity statement, stale payer policy, unauthorized chart access, unsafe clinical recommendation without clinician review
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real healthcare prior authorization workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
