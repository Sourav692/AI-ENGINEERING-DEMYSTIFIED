# Full Premium Mock Interview: Agentic Support Workflow
...
## Customer scenario
A marketplace company wants an agent that reads support tickets, checks order status, drafts replies, applies tags, and sometimes issues credits. The demo looked impressive, but in pilot the agent refunded the wrong order twice and sent a confident reply based on a stale shipping event.

## Ambiguous customer request
> “We want the AI to handle support tickets end-to-end so agents can focus on difficult customers.”

## Strong candidate target framing
- Primary workflow: triage and draft resolution for delivery delays, refund eligibility, missing item claims, and account access tickets
- Users: frontline support agents, support managers, trust and safety reviewers, finance operations, customer experience leaders
- Success metrics: ticket handle time, auto-tag precision, draft acceptance rate, refund error rate, escalation quality, customer CSAT, policy violation rate, p95 end-to-end latency
- Architecture direction: ticket webhook, case classifier, retrieval from policy/versioned macros, order-status tools, action policy engine, tool permission service, draft generator, approval UI, refund workflow with thresholds, event logs and replay
- Security boundary: tool allowlists by role, refund approval thresholds, PII minimization, customer-account isolation, policy conflict handling, audit trail, malicious ticket-content filtering
- Latency thinking: classify and fetch order status in parallel, reserve slow reasoning for high-risk cases, provide draft within 8 seconds, background refresh for tracking events, asynchronous manager approval for refunds
- Cost thinking: use deterministic rules for tagging where possible, small model for classification, larger model only for complex draft generation, budget per ticket, cache policy snippets by version
- Rollout direction: shadow mode on 1,000 historical tickets, draft-only pilot, approval-required refunds below small threshold, then narrow automation for low-risk cases
...
**Interviewer:** That sounds high level. The customer says: “We want the AI to handle support tickets end-to-end so agents can focus on difficult customers.” What do you do in the first meeting?
...
**Candidate:** For this case, I would focus the first release on triage and draft resolution for delivery delays, refund eligibility, missing item claims, and account access tickets. The relevant users are frontline support agents, support managers, trust and safety reviewers, finance operations, customer experience leaders. The first dashboard should track ticket handle time, auto-tag precision, draft acceptance rate, refund error rate, escalation quality, customer CSAT, policy violation rate, p95 end-to-end latency. The security controls should emphasize tool allowlists by role, refund approval thresholds, PII minimization, customer-account isolation, policy conflict handling, audit trail, malicious ticket-content filtering. I would not index every possible source at the beginning; I would start with authoritative sources and named owners.
...
**Candidate:** I would agree that this is a real risk and explain the controls. The LLM should only receive context that the user is authorized to see. Permissions must be enforced in retrieval and tool execution, not only in the prompt. Logs should avoid storing sensitive raw content unless required and protected. We need audit trails, data retention controls, prompt-injection tests, and clear blocked actions. For this mock, the core security boundary is: tool allowlists by role, refund approval thresholds, PII minimization, customer-account isolation, policy conflict handling, audit trail, malicious ticket-content filtering.
...
**Candidate:** I would avoid arguing in abstract. I would calculate cost per successful workflow outcome. For this use case, the cost strategy is: use deterministic rules for tagging where possible, small model for classification, larger model only for complex draft generation, budget per ticket, cache policy snippets by version. I would show a cost dashboard with request volume, model distribution, average tokens, tool calls, cache hit rate, and cost per accepted answer or resolved case. If the assistant saves time but costs more than the value created, the architecture is wrong.
...
**Candidate:** I would push back respectfully. I would say we can launch a controlled pilot next Monday, but not broad production unless the launch gates are met. My rollout would be: shadow mode on 1,000 historical tickets, draft-only pilot, approval-required refunds below small threshold, then narrow automation for low-risk cases. I would also define rollback conditions, such as leakage, high-risk wrong answers, excessive latency, or low user trust.
...
**Weak candidate:** I would build an agent with tools for Zendesk, order lookup, and refund. The agent would decide what to do and call the right tool. We can monitor errors after launch.
...
**Average candidate:** I would split tickets by intent, retrieve support policy, call order tools, and require human approval for refunds. I would evaluate draft quality and tool-call correctness before launch.
...
**Candidate:** I would not let an LLM freely operate the support stack. I would separate classification, evidence gathering, draft generation, and action execution. The policy engine, not the model, decides whether a refund can happen. The model proposes; guarded services execute only after permission, threshold, and evidence checks pass.
