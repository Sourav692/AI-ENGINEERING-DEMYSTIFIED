# Retail Demand Forecast Explanation Assistant - Answer Key
...
- What exact retail demand forecasting and planning workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: planner reviews forecast variance, asks why demand changed, assistant explains drivers using sales history, promotions, weather, inventory, events, and recommends human-reviewed planning actions.
...
- Connectors ingest and normalize data from forecasting platform, POS sales, inventory, promotions calendar, pricing system, weather/events feeds, ERP, BI dashboards; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
- SKU(id, category, brand, lifecycle_stage); Store(id, region, format); Forecast(id, sku_id, store_id, horizon, prediction, confidence); ActualSales(id, sku_id, store_id, date, units); DriverSignal(id, type, value, source, timestamp); Explanation(forecast_id, top_drivers, confidence, caveats).
...
## Red-team risks
- confusing correlation with causation, hallucinated drivers, stale promotions data, biased recommendations against stores/regions, over-ordering due to overconfident explanation
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real retail demand forecasting and planning workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
