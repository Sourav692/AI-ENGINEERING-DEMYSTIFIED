# Multi-Tenant SaaS Support Assistant - Answer Key
...
- What exact multi-tenant SaaS customer support workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: agent handles tenant-specific support question, retrieves tenant entitlement/config/log context, drafts response, opens escalation if needed, never exposing another tenant’s data.
...
- Connectors ingest and normalize data from ticketing, tenant config DB, product telemetry/logs, docs, CRM, billing/entitlements, identity provider, data warehouse; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
- Tenant(id, plan, region, data_residency, features); User(id, tenant_id, role); Ticket(id, tenant_id, issue_type, status); Entitlement(tenant_id, feature, limits); ProductEvent(id, tenant_id, user_id, event_type, timestamp); KnowledgeArticle(id, product_area, visibility); RetrievalACL(tenant_id, role, doc_id).
...
## Red-team risks
- cross-tenant leakage, wrong entitlement advice, exposing internal logs, prompt injection in ticket text, automated refund/credit actions without approval
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real multi-tenant SaaS customer support workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
