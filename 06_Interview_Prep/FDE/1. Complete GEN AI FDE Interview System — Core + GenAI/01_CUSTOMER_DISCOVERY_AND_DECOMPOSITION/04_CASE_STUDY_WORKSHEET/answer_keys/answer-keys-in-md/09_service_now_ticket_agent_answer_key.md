# ServiceNow Ticket Automation Agent - Answer Key
...
- What exact ITSM ticket automation workflow is slow, risky, or inconsistent today, and what decision does the user need to make at the end of the workflow?
...
- Support the core workflow: new ticket arrives, agent classifies category/priority, retrieves KB/runbook, asks missing questions, drafts response, proposes assignment, and only performs workflow actions within policy.
...
- Connectors ingest and normalize data from ServiceNow, CMDB, knowledge base, identity/access system, email/chat, monitoring alerts, approval workflow; ingestion preserves metadata, document version, source owner, freshness, and ACLs.
...
- Ticket(id, requester, category, priority, description, status, assignment_group); Asset(ci_id, owner, service, criticality); KBArticle(id, category, validity, approval_status); AssignmentRule(id, condition, group, confidence); WorkflowAction(id, ticket_id, action, approval_required, executed_by); AuditLog(ticket_id, model_output, source_ids).
...
## Red-team risks
- wrong priority/assignment, closing tickets incorrectly, unauthorized access changes, leaking requester PII, prompt injection in ticket description
...
## Weak answer
I would connect the documents to a vector database, use an LLM to answer questions, and maybe add a chatbot UI. This is weak because it ignores the real ITSM ticket automation workflow, permission boundaries, source freshness, evaluation, human approval, and production monitoring.
