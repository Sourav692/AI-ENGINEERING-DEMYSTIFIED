...
Everything that is not one of the tutorials above is grouped by topic. Reorganised
2026-09-19: three of these already existed while three equally-sized clusters sat loose
at the top level, and none of the six were listed here.

| Folder | What's in it |
|---|---|
| [`Cost_Latency_Optimization/`](Cost_Latency_Optimization/) | The largest body of work here — the GenAI FDE cost/latency playbook PDF, two cram sheets (full playbook + §15–16), `CORE_8_DRIVERS_MEMORIZE.md`, additions beyond the playbook, and two drill decks |
| [`chunking/`](chunking/) | Two complementary docs, md + html each. `chunking-reference-by-doc-type` is the reference guide (per-type breakdown, comparison table, decision framework, Databricks notes); `chunking-lab-and-drills` is the lab writeup plus 5 interview drills. Renamed 2026-09-19 — they were `chunking-by-doc-type` and `01-chunking-strategies-by-doc-type`, near-identical names for different documents |
| [`retrieval_strategies/`](retrieval_strategies/) | `rag_retrieval_strategies` — retrieval strategy walkthrough, md + html |
| [`multi_agent/`](multi_agent/) | `multi_agent_coordination_patterns` (md + html) and `multi_agent_systems_qa_reference.html`. Companion material to tutorial 07, at more depth |
| [`callbacks/`](callbacks/) | Callbacks in agentic systems: the LangGraph + OpenAI tutorial PDF and its companion notebook `LangGraph_Agent_Callbacks_OpenAI.ipynb` |
| [`reference/`](reference/) | Two standalone PDFs kept for reading, not generated from this repo: the AI Engineer interview kit (RAG/agents/production failure scenarios) and *Async Operation in AI Agents* |

**`multi_agent_systems_qa_reference.html` has no markdown source** — it is the only file
here that cannot be regenerated if the format changes. Worth knowing before editing it.
