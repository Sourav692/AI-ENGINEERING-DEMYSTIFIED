...
Keep this gateway distinct from the "deterministic gateway" in `Case_Study_Groups/G02_Customer_Support_Automation.md` section 6. That one decides which *actions* an agent may take. This one decides how *model traffic* is screened, paid for and recovered. A real support product has both.
...
A self-hosted model changes the cost model from tokens to GPU-hours. Size it with the air-gapped chapter's formula, `Replicas = ⌈(QPS × Tokens_request) / (TokensPerSecond_replica × UtilizationTarget)⌉`. When the answer exceeds the available hardware, change the workload, not the hardware request. `Case_Study_Groups/G21_LLM_Inference_Serving.md` covers batching and failover inside that pool, and `Case_Study_Groups/Standalone/19_Air_Gapped_AI_System/` covers the fully disconnected case.
...
| Cross-references | `Case_Study_Groups/G02_Customer_Support_Automation.md` section 6 (the action gateway, a different component); `G07_Secure_Multi_Tenant_AI_Platform.md` sections 5 and 8 (tenant context, regional routing); `G14_Observability_And_Production_Diagnosis.md` sections 6, 7 and 11; `G19_Consumer_Scale_Chat_Service.md` sections 7 and 11 (cascade cost, regional failover); `G21_LLM_Inference_Serving.md` (self-hosted serving); `Standalone/19_Air_Gapped_AI_System/` |
