...
| Section | Notebook | Topic |
|---|---|---|
| `01_Tool_Use/` | `01_Tool_Use_Agentic_Systems.ipynb`, `02_Tool_Calling_vs_ReAct.ipynb`, `03_Tool_Use_Alt.ipynb`, `04_ReAct_Alt.ipynb`, `05_SWE_Agent_Applied.ipynb`, `06_BrowserAgent_Computer_Use_Applied.ipynb`, `07_Hosted_vs_Client_Side_Tools.ipynb`, `08_Vision_Driven_Computer_Use.ipynb`, `09_Hosted_Code_Execution.ipynb`, `10_Agentic_Image_Generation.ipynb` | Tool-use strategies. `02_` compares loop *shapes*; `07_` (added 2026-09-20) compares execution *locations* — the same capability run client-side via `chat.completions` and hosted via `responses.create`, with the seam you only get client-side. `06_` is a deliberate simulation with *text* observations; `08_` (added 2026-09-20) is the pixel version — real PNG screenshots to a vision model, coordinates back, plus a frontend-testing section that points the agent at a broken button and asserts on the result. Pillow renders the UI, so no browser dependency was added. `09_` (added 2026-09-20) is hosted code execution — the model writes Python, runs it in a provider sandbox, and hands back the source it ran, which is the counter-example to `07_`'s "you cannot see a hosted tool's result". `10_` (added 2026-09-20) is the artefact case — the agent decides to generate an image, writes its own prompt (`revised_prompt`), and then *describes its own output as correct without looking at it*; the notebook catches that by feeding the PNG back to a vision model, which is evaluator-optimizer with a vision evaluator |
...
**Still open:**
- ~~Notebooks without a `# Title` first markdown cell~~ — **fixed 2026-09-19.** 142 notebooks were brought into line: 128 got a title cell prepended (derived from the filename, nothing existing touched) and 14 had a `##`/`###`/`####` heading that already named the notebook demoted to `#`. Five notebooks that title themselves with a styled HTML `<h1>` were left as their authors wrote them. All 526 now open on a title, and `scripts/check_repo_invariants.py` enforces it on every commit.
...
- ~~Relative `../../data/` paths broken post-move~~ — **fixed 2026-09-19.** The five `06_RAG_Naive_to_Production/` notebooks were repointed to `../../shared_data/`, and the 28 `Comprehensive_RAG_Techniques/all_rag_techniques/` notebooks plus one runnable script to `../data/`. All 140 data references in the repo now resolve; verified by walking every code cell.
